...
async function getStats(req, res) {
    try {
        const [totalOrders, pendingOrders, paidRevenue, productStatus,] = await Promise.all([
        ...
        const topProductsRaw = await prisma_1.default.orderItem.groupBy({
            by: ["productId"],
            _sum: { quantity: true },
            where: {
                order: {
...
async function createOrder(req, res) {
    try {
        const { items, customerName, customerPhone, customerEmail, deliveryAddress, deliveryMethod, notes, } = req.body;
        // 🔹 Validar carrito
...
async function updateOrderStatus(req, res) {
    try {
        ...
        const allowedTransitions = {
            ...
            [client_1.OrderStatus.CONFIRMED]: [client_1.OrderStatus.DELIVERED],
            [client_1.OrderStatus.DELIVERED]: [],
        ...
        if (order.status === client_1.OrderStatus.PAID &&
            nextStatus === client_1.OrderStatus.CONFIRMED) {
            // 1️⃣ Validar stock
            ...
            // 2️⃣ Descontar stock
            for (const item of order.items) {
                await prisma_1.default.$transaction(order.items.map(item => prisma_1.default.product.update({
                    where: { id: item.productId },
                    data: {
                        stock: {
